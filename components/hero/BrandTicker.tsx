...
export const BrandTicker: React.FC = () => {
  const brands = [
    ...
    },
    {
      name: 'Netflix',
      logo: (
        <svg className="h-4.5 w-auto" viewBox="0 0 24 24" fill="#E50914">
    ...
    },
    {
      name: 'Adobe',
      logo: (
        <svg className="h-4.5 w-auto" viewBox="0 0 24 24" fill="#FF0000">
  ...
  return (
    <div className="py-4 px-6 rounded-2xl bg-zinc-900/60 border border-zinc-800/80 backdrop-blur-md">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="text-xs">
            <span className="font-bold text-white">Live Bot Dispatch</span>
            <span className="text-zinc-500 ml-2">&lt; 30s latency</span>
          </div>
        </div>

        {/* Real Brand Logos Showcase */}
        <div className="flex items-center gap-8 overflow-x-auto scrollbar-none py-1.5 px-2">
          {brands.map((brand, idx) => (
            <div
              key={idx}
              className="flex items-center gap-2 text-zinc-400 hover:text-white transition-all hover:scale-110 cursor-pointer shrink-0"
              title={brand.name}
            >
              {brand.logo}
        ...
        {/* Warranty Badge */}
        <div className="hidden lg:flex items-center gap-2 text-xs text-zinc-300 shrink-0 font-medium">
          <ShieldCheck className="h-4 w-4 text-emerald-400" />
          <span>Full-Term Guarantee</span>
        </div>
