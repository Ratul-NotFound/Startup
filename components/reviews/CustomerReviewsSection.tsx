    ...
    <section className="relative w-full space-y-5 pt-4 pb-8" id="reviews-section" suppressHydrationWarning>
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-slate-200 dark:border-white/[0.08] pb-5">
        <div className="space-y-1.5">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-cyan-50 dark:bg-zinc-900 border border-cyan-200 dark:border-white/10 text-cyan-700 dark:text-cyan-400 text-[11px] font-semibold">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            <button
              type="button"
              onClick={() => scroll('left')}
              className="p-2 rounded-xl bg-white hover:bg-slate-50 dark:bg-zinc-900 dark:hover:bg-zinc-800 text-slate-700 dark:text-zinc-300 hover:text-slate-950 dark:hover:text-white border border-slate-200 dark:border-white/10 transition-colors shadow-sm cursor-pointer"
              title="Scroll Left"
            >
          ...
          <motion.button
            ...
            className="px-4 py-2 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-zinc-950 hover:bg-slate-800 dark:hover:bg-zinc-100 font-bold text-xs flex items-center justify-center gap-1.5 transition-all shadow-sm cursor-pointer"
          >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all whitespace-nowrap cursor-pointer ${
                activeCategory === cat.id
                  ? 'bg-slate-900 dark:bg-zinc-800 text-white border border-slate-900 dark:border-white/20 shadow-sm'
                  : 'text-slate-600 dark:text-zinc-400 hover:text-slate-950 dark:hover:text-zinc-200 hover:bg-slate-100 dark:hover:bg-zinc-900/80 border border-transparent'
              }`}
            >
              {cat.label}
            </button>
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1 transition-all shrink-0 cursor-pointer ${
            starFilter === 5
              ...
              : 'text-slate-600 dark:text-zinc-400 hover:text-slate-950 dark:hover:text-zinc-200 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-white/5 shadow-sm'
          }`}
        >
            ...
            filteredReviews.map((rev) => {
              ...
              return (
                <motion.div
                  ...
                  className="w-[290px] sm:w-[320px] shrink-0 snap-start p-4 rounded-2xl bg-white dark:bg-zinc-900/95 border border-slate-200 dark:border-white/10 hover:border-cyan-500/40 transition-all duration-300 hover:shadow-[0_8px_30px_rgba(6,182,212,0.12)] shadow-sm dark:shadow-none flex flex-col justify-between space-y-3 contain-card"
                >
                  <div className="space-y-2.5">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-1">
                            <span className="font-bold text-xs text-slate-900 dark:text-white truncate">{rev.userName}</span>
                            {rev.verifiedPurchase && (
                              <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold shrink-0">✓</span>
                            )}
                          </div>
                          <span className="text-[10px] text-slate-500 block leading-tight">{getRelativeTime(rev.createdAt)}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-0.5 text-amber-500 shrink-0">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <Star
                            key={s}
                            className={`h-3 w-3 ${s <= rev.rating ? 'fill-amber-400' : 'text-slate-300 dark:text-zinc-700'}`}
                          />
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-slate-100 dark:bg-zinc-800/80 border border-slate-200 dark:border-white/5 text-[10px] text-slate-700 dark:text-zinc-300 max-w-full truncate">
                      ...
                      {rev.planDuration && (
                        <span className="text-slate-500 text-[9px] shrink-0">• {rev.planDuration}</span>
                      )}
                    </div>

                    {/* Review Title & Comment */}
                    <div className="space-y-0.5">
                      <h4 className="text-xs font-bold text-slate-900 dark:text-white line-clamp-1 leading-snug">{rev.title}</h4>
                      <p className="text-[11px] text-slate-600 dark:text-zinc-400 line-clamp-2 leading-relaxed">{rev.comment}</p>
                    </div>
                  </div>
