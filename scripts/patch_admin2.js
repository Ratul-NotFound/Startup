/**
 * Patch 2: Fix misplaced code and update the old provision modal
 */
const fs = require('fs');
...
function findLine(pattern, startFrom = 0, endAt = lines.length) {
  for (let i = startFrom; i < Math.min(endAt, lines.length); i++) {
    if (lines[i].includes(pattern)) return i;
  }
  return -1;
}

// ─── Fix 1: Remove misplaced paymentVerifiedAt code at line ~853 ──────────
{
  const badLine = findLine("o.paymentVerifiedAt && o.paymentStatus === 'pending'", 845, 870);
  if (badLine >= 0) {
    // Remove the 8-line block (lines 853-860 0-indexed 852-859)
    lines.splice(badLine, 8);
    console.log("✓ Fix 1: removed misplaced paymentVerifiedAt block at line", badLine+1);
  } else console.error("✗ Fix 1 pattern not found");
}

// ─── Fix 2: Add paymentVerifiedAt to the right place (status <td> in orders table) ──
{
  // Find the correct status span in orders table
  // Look for `{o.paymentStatus}` inside the orders table area (after line 1950 approx)
  const statusSpan = findLine('{o.paymentStatus}', 1960);
  if (statusSpan >= 0) {
    const tdClose = findLine('</td>', statusSpan + 1);
    if (tdClose >= 0) {
      ...
      console.log("✓ Fix 2: added status indicators at correct location, line", tdClose+1);
    } else console.error("✗ Fix 2 tdClose not found");
  } else console.error("✗ Fix 2 paymentStatus span not found after line 1960");
}

// ─── Fix 3: Update old provision modal to use perItemCreds ────────────────
// Find the custom provision modal section and replace it
{
  const modalStart = findLine("customProvisionOrder &&", 4100);
  if (modalStart < 0) { console.error("✗ Fix 3 modal start not found"); }
  else {
    // find end: two consecutive </div> closes
    let modalEnd = findLine("setCustomProvisionOrder(null)", modalStart + 1);
    if (modalEnd < 0) { console.error("✗ Fix 3 modal end not found"); }
    else {
      // find the outer closing of the modal
      let outerClose = findLine(")}", modalEnd + 1);
      console.log("Modal start:", modalStart+1, "seCustom:", modalEnd+1, "outerClose:", outerClose+1);
      
      // Replace the entire modal
      lines.splice(modalStart, outerClose - modalStart + 1,
      ...
      console.log("✓ Fix 3: replaced provision modal");
    }
  }
}
...
console.log('\n✅ Patch 2 complete. Total lines:', lines.length);
